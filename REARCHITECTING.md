...
## Goals
- Minimal frontend data: push only what's needed. Ticks are pushed on a fixed ~1000 ms cadence (1 Hz), coalesced; not on every tick. A lightweight daily % change is included within each tick payload.
...
- Bid price stream (1000ms): values come from `mt5.symbol_info_tick` and should match MT5 Market Watch bid exactly at the same moment; minor differences can occur due to transmission delay and local rounding. Tolerance: 0 pips ideally; up to 1 pip during fast markets.
...
| 24 | WS-V2-7 | WebSocket v2 | Remove per-client subscription model post cutover | Backend | DONE | Deleted subscribe/unsubscribe handlers; removed `SubscriptionInfo`-based gating; v2 is broadcast-only | `server.py`,`app/models.py` | WS-V2-5 | Ping/pong kept; global shaping defaults retained |
| 25 | DOC-3 | Docs | Update examples to broadcast-only (remove sub flows) | Backend | DONE | Update `README.md` and `test_websocket.html` to reflect v2 broadcast; note legacy endpoints until removal | `README.md`,`test_websocket.html` | WS-V2-7 | Keep optional snapshot subscribe notes |
