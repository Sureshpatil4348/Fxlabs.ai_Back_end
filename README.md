...
- **Subscription Gate (Email)**: Before sending an email, the backend calls the Supabase Edge Function `subscription_check-by-email` with `SUPABASE_PUBLIC_ANON_KEY`; only a successful response with `"subscription_status":"expired"` suppresses the email (any errors fail-open and still send).
