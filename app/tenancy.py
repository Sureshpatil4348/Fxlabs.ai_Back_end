...
def load_tenant_config() -> TenantConfig:
    ...
    if tenant == "FXLabs":
        return TenantConfig(
            ...
            # Email: strictly tenant-specific (no global fallbacks or defaults)
            sendgrid_api_key=_get("FXLABS_SENDGRID_API_KEY", ""),
            from_email=_get("FXLABS_FROM_EMAIL", ""),
            from_name=_get("FXLABS_FROM_NAME", ""),
    ...
    return TenantConfig(
        ...
        # Email: strictly tenant-specific (no global fallbacks or defaults)
        sendgrid_api_key=_get("HEXTECH_SENDGRID_API_KEY", ""),
        from_email=_get("HEXTECH_FROM_EMAIL", ""),
        from_name=_get("HEXTECH_FROM_NAME", ""),
